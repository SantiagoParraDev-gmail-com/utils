...
describe("classSet()", () => {
  ...
  describe("add()", () => {
    it("handles strings", () => {
      ...
      expect(classSet({ foo: true }).add("bar").toString()).toBe("foo bar");
    });

    it("handles objects", () => {
      expect(classSet().add({ foo: true }).toString()).toBe("foo");

      expect(classSet("foo").add({ bar: true }).toString()).toBe("foo bar");
  ...
  describe("invert()", () => {
    ...
    it("handles objects", () => {
      ...
      expect(classSet({ foo: true, bar: false }).invert().toString()).toBe(
        "bar"
      );
    });
    ...
    it("handles strings and objects", () => {
      expect(
        classSet({ foo: true, bar: false }).add("baz").invert().toString()
      ).toBe("bar");
    });
  });
});
